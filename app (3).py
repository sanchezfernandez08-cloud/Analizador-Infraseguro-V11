...
with tab_calc:
    ...
    # ════════════════════════
        ...
        pct_comun_bc, sup_comun_bc, sup_units_bc = widget_distribucion_superficies(sup_total_bc)
        pct_bc_pct = int(pct_comun_bc * 100)

        # Indicar claramente la superficie que se usará en el cálculo
        if sup_total_bc and sup_comun_bc:
            st.success(
                f"✅ Se calculará usando **{sup_comun_bc:,.0f} m²** "
                f"({pct_bc_pct}% de {sup_total_bc:,.0f} m² totales) "
                f"como superficie de bienes y espacios comunes."
            )
